...
export default function AdminApp() {
  ...
  const fetchCourses = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        setCourses(data);
        if (selectedAppCategory !== "All Applications" && !data.some((c: Course) => c.category === selectedAppCategory)) setSelectedAppCategory("All Applications");
      } else {
        setCourses([]);
      }
    } catch (e) {
  ...
  const handleDeleteCourse = async (courseId: string) => {
    try {
      const response = await fetchWithRole(`/api/courses/${courseId}`, { method: 'DELETE' });
      if (!response.ok) { const data = await response.json(); throw new Error(data.error || 'Course could not be deleted.'); }
      await fetchCourses();
        ...
        <div className="w-full max-w-md bg-white rounded-2xl border border-slate-200 shadow-xs p-8 space-y-5">
          ...
          <div className="p-3.5 rounded-xl bg-indigo-50/80 border border-indigo-100 text-xs text-indigo-900 space-y-1">
            ...
            <p className="text-indigo-800/90 leading-relaxed">
              Use your master recovery key (<code className="bg-indigo-100 px-1.5 py-0.5 rounded font-mono text-[11px] font-bold text-indigo-950">server recovery key</code> or custom <code className="bg-indigo-100 px-1.5 py-0.5 rounded font-mono text-[11px]">ADMIN_RESET_TOKEN</code>).
            </p>
          </div>
